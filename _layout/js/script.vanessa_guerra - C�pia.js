...
var galeria = {
	...
	init			: function(){ 
		...
		galeria.scroller.find('img , em').bind('click',function(){
			if ( $(this).is('img') ) 	var url = $(this).attr('src'); //pega url
			else						var url = $(this).siblings('img').attr('src'); //pega url
			...
			if ( url.search('youtube') > 0 ){ 
				url = url.split('/');
				$('#galeria_multimidia .stage em').html('&nbsp;<iframe width="560" height="315" src="http://www.youtube.com/embed/'+ url[4] +'?wmode=opaque" frameborder="0" allowfullscreen></iframe>&nbsp;');
			}
			// vimeo
			else if ( url.search('vimeo') > 0 ){ 
				url = url.split('/');
				$('#galeria_multimidia .stage em').html('&nbsp;<iframe src="http://player.vimeo.com/video/'+ url[4] +'" width="560" height="315" frameborder="0" webkitAllowFullScreen mozallowfullscreen allowFullScreen></iframe>&nbsp;');
			}
			// fotos
			else {
				url = url.replace('_tb','');
			...
			if (!site.isiPad) index = $(this).parent().parent().index();
			...
			else if ( index == galeria.itens-1 )	galeria.stage.proximo.addClass('inactive'); //se for a última foto
			else									$('#galeria_multimidia .stage a').removeClass('inactive');
		});
	},
	navClick		:	function(){
		// ---------------- botões prev / next do carousel
		galeria.carousel.proximo.bind('click',function(){
			galeria.scroller.stop(true,true).animate({ scrollLeft: galeria.scroller.scrollLeft() + galeria.item_width },'fast', galeria.testeScroll );
			return false;
		});
		...
		galeria.stage.proximo.bind('click',function(){
			...
			if ( selected.parent().next().length > 0 ) {
				selected.removeClass('selected').parent().next().children().children('img').trigger('click');
				// se a posição do item selecionado for menor que a posição do scroll (esquerda)
				if ( pos <  galeria.scroller.scrollLeft() )
					galeria.scroller.stop(true,true).animate({ scrollLeft: pos },'fast', galeria.testeScroll );
				// se a posição do item selecionado for maior que a posição do scroll (direita)
					...
					galeria.scroller.stop(true,true).animate({ scrollLeft: selected.parent().next().index() * galeria.item_width + galeria.item_width - galeria.scroller.width()  },'fast', galeria.testeScroll );
				}
			}
			return false;
		});
		galeria.stage.anterior.bind('click',function(){
			...
			if ( selected.parent().prev().length > 0 ) {
				selected.removeClass('selected').parent().prev().children().children('img').trigger('click');
				// se a posição do item selecionado for maior que a posição do scroll (direita)
				if ( pos >  galeria.scroller.scrollLeft() + galeria.scroller.width() )
					galeria.scroller.stop(true,true).animate({ scrollLeft: pos + galeria.item_width - galeria.scroller.width() },'fast', galeria.testeScroll );
				// se a posição do item selecionado for menor que a posição do scroll (esquerda)
				else if ( pos <=  galeria.scroller.scrollLeft() )
					galeria.scroller.stop(true,true).animate({ scrollLeft: selected.parent().prev().index() * galeria.item_width  },'fast', galeria.testeScroll );
			}
			return false;
		});
	},
	navSwipe		: function(){
		...
		galeria.stage.proximo.bind('click',function(){
			...
			if ( selected.next().length > 0 ) 
				selected.removeClass('selected').next().children('img').trigger('click');
			else if ( selected.parent().next().length > 0 )
				selected.removeClass('selected').parent().next().children(':first-child').children('img').trigger('click');
		...
		galeria.stage.anterior.bind('click',function(){
			...
			if ( selected.prev().length > 0 )
				selected.removeClass('selected').prev().children('img').trigger('click');
			else if ( selected.parent().prev().length > 0 )
				selected.removeClass('selected').parent().prev().children(':last-child').children('img').trigger('click');
...
var topo = {
	...
	resize	:	function(){
		if ($(window).width() - topo.scroller	.width() > 0 ) {
			topo.scroller.css({ paddingLeft :  ($(window).width() - topo.scroller	.width()) /2 ,
													paddingRight : ($(window).width() - topo.scroller.width()) /2 });
		}
		else {
			topo.scroller.css({ paddingLeft :  0 ,
													paddingRight : 0 });
		}
	},
	detalhes	:	function(){
		topo.lista.find('li:eq(1)').bind('mouseenter',function(){
			...
			$(this).find('div').stop(true,true).fadeOut(site.animacao/2);
		});
	},
...
function f_Projetos() {
	if (!site.isiPad){
		...
		$('#projetos .descricao > a').live('click',function(){
...
function ajaxInit(){
	...
	if (modulo!='home') {
		$.ajax({
			type: 'POST',
			url: 'json.php',
			data: {modulo:modulo, classe:classe, variaveis:variaveis},
			dataType: 'json',
			beforeSend: function(){
				//adiciona o loader
					...
					if ( !classe ){
						...
						for ( i in resposta.projetos ) {
							...
							for ( j in resposta.projetos[i][0].imagens ) {
								...
								if ( !site.isiPad ) {
									itens += '<li><div><img src="thumb/phpThumb.php?src=../galeria/projetos/'+ resposta.projetos[i][0].imagens[j].foto +'&w=140&h=86"alt="" /></div></li>';
									continue;
								}
								aux++;
								if ( aux == 1 ) { itens += '<li>'; }
								itens += '<div><img src="thumb/phpThumb.php?src=../galeria/projetos/'+ resposta.projetos[i][0].imagens[j].foto +'&w=140&h=86" alt="" /></div>';
								if (aux == 6) { itens += '</li>'; aux = 0; }
							}
							for ( j in resposta.projetos[i][0].videos ) {
								legenda += '<li>'+ resposta.projetos[i][0].videos[j].legenda +'</li>';
								if ( !site.isiPad ) {
									itens += '<li><div><em></em><img src="http://img.youtube.com/vi/'+ resposta.projetos[i][0].videos[j].video +'/1.jpg" height="86" alt="" /></div></li>';
									continue;
								}
								aux++;
								if ( aux == 1 ) { itens += '<li>'; }
								itens += '<div><em></em><img src="http://img.youtube.com/vi/'+ resposta.projetos[i][0].videos[j].video +'/1.jpg" height="86" alt="" /></div>';
								if (aux == 6) { itens += '</li>'; aux = 0; }
							}
