...
/**
* forma a imagem para ser mostrada no front do site
*/
function formThumbImage($file,$id,$params="",$qualidadeJPEG=80,$color="666666")
{
    global $endereco_site;
    global $db;

    $dadosImagem["id"] = $id;
    $dadosImagem["arquivo"] = $file;

    $preencherURL = "&far=1&bg=".$color;

    $mysql = $db->prepare('SELECT * FROM crops WHERE relacao_id="'.$dadosImagem["id"].'" ORDER BY id DESC LIMIT 1;');
    $mysql->execute();
    $resultadoCrop = $mysql->fetchAll();
    $resultadoCrop=end($resultadoCrop);

    // if (!empty($resultadoCrop)  && ($resultadoCrop["altura"] || $resultadoCrop["largura"])) {
    // 	return "plugins/thumb/phpThumb.php?src=../../galeria/".$dadosImagem["arquivo"]."$params&zc=1&aoe=1&q=".$qualidadeJPEG."&sx=".$resultadoCrop["x"]."&sy=".$resultadoCrop["y"]."&sw=".$resultadoCrop["largura"]."&sh=".$resultadoCrop["altura"].$preencherURL;
    // } else {
    // 	return "plugins/thumb/phpThumb.php?src=../../galeria/".$dadosImagem["arquivo"]."$params&q=".$qualidadeJPEG."".$preencherURL;
    // }
    if (!empty($resultadoCrop)  && ($resultadoCrop["altura"] || $resultadoCrop["largura"])) {
        return "plugins/thumb/phpThumb.php?src=../../galeria/".$dadosImagem["arquivo"]."$params&zc=1&aoe=1&q=".$qualidadeJPEG."&sx=".$resultadoCrop["x"]."&sy=".$resultadoCrop["y"]."&sw=".$resultadoCrop["largura"]."&sh=".$resultadoCrop["altura"].$preencherURL;
    } else {
...
* forma a imagem para ser mostrada no front do site
*/
function clearformThumbImage($file,$id,$params="",$qualidadeJPEG=80,$color="666666")
{
    global $endereco_site;
    global $db;
    ...
    $query = 'SELECT * FROM crops WHERE relacao_id="'.$dadosImagem["id"].'" ORDER BY id DESC LIMIT 1;';
...
function formaURL($str, $enc = 'UTF-8')
{
    $acentos = array(
    ...
    if ($total_paginas>1) {
        ...
        <ol class="paginador">
            <?php
                if ($mostraSetas) {
                    ?>
                    <li class="seta"><button value="<?=$linkPaginador;?>/<?=$paginaAtual-1;?>"<?php if ($paginaAtual<=1) { ?> disabled="disabled"<?php } ?>>Anterior</button></li>
                    <?php
                }
                for ($p=1;$p<=$total_paginas;$p++) {
                    ?>
                    <li><button value="<?=$linkPaginador;?>/<?=$p;?>"<?php if ($paginaAtual==$p) { ?> disabled="disabled"<?php } ?>><?=$p;?></button></li>
                    <?php
                }
                if ($mostraSetas) {
                    ?>
                    <li class="seta"><button value="<?=$linkPaginador;?>/<?=$paginaAtual+1;?>"<?php if ($paginaAtual>=$total_paginas) { ?> disabled="disabled"<?php } ?>>Próxima</button></li>
                    <?php
                }
            ?>
        </ol>
