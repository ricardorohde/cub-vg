...
{
	function salvarCover()
	{
		postRequest();
		$dadosJson=readJSON($_POST["ajaxACK"]);

		$id = $dadosJson["foto_id"];
		$cover = $dadosJson["cover"];
		$toUnsetIds = $dadosJson["ids"];

		if(!empty($toUnsetIds));
		foreach($toUnsetIds as $unsetId) {

			$dadosFoto = array();
			$dadosFoto["resultados"]["id"]=$unsetId;
			$dadosFoto["resultados"]["cover"]=0;

			dbUpdate("fotos", $dadosFoto);
		}

		$dadosFoto = array();
		$dadosFoto["resultados"]["id"]=$id;
		$dadosFoto["resultados"]["cover"]=$cover;
	...
	function rec_senha ($dadosUser) {
		...
		if ($existe) {
			...
			if (enviaEmail($dadosUser["email"], "ACKrec_senha", $existe)) {
				dbUpdate("usuarios", array("resultados"=>array("senha"=>md5($novaSenha), "id"=>$existe["id"])));
	...
	function carregar_mais($dadosJSON) {		
		...
		switch($modulo){
			...
			case 'produtos':
				...
				//Chama o Model Geral para listar os tópicos
				$modelGeral=new ACKgeral_Model();
				$produtos=$modelGeral->listaItens("produtos", $itens,$limite,"titulo_pt",$categoriaProduto,$order);
			...
			case 'categoriasProdutos':
				...
				//Carrega o ID do Módulo
				$moduloProdutos=$modelSite->idModulo("categorias_produtos",false);
