...
class ACKprodutos_Controller
{
    public function index()
    {
        ...
        $moduloCategoriasProdutos = $modelSite->idModulo("categorias_produtos",false);
        ...
        listaCategoria($moduloCategoriasProdutos,"0",$arrayCategoria);
    ...
    public function excluir($dadosJSON)
    {
        postRequest();
    ...
    public function incluir()
    {
        ...
        //Carrega o ID do Módulo
        $moduloVerifica=$modelSite->idModulo("categorias_produtos",false);
    ...
    public function editar($dadosURL)
    {
        $id=$dadosURL[0];
        protectedArea();
        openSession();
    ...
    ## CATEGORIAS
    ###############################################################################
    public function categorias($dadosURL)
    {
        $acao=$dadosURL[0];
        if ($acao!="") {
    ...
    public function editarCategoria($dadosURL)
    {
        $id=$dadosURL[1];
        protectedArea();
        openSession();
        ...
        $arrayCategoria=array();
        array_push($arrayCategoria, array('id'=>"0", 'nome'=>"Nível principal", 'nivel'=>"0"));
        listaCategoria($moduloVerifica,"0",$arrayCategoria,0);
        $dados["categoriasSelect"]=$arrayCategoria;
    ...
    public function salvarCategoria()
    {
        ...
        if (!$modelUser->userLevel($moduloVerifica,false,"2")) {
            ...
            if ($dadosJSON["acao"]=="incluir") {
                ...
                $dados["resultados"]["visivel"]=$dadosJSON["visivel"];
                $dados["resultados"]["modulo"]=$moduloVerifica;
                $dados["resultados"]["status"]="1";
                $ordem = proximaOrdem("categorias", array("modulo"=>$moduloVerifica, "relacao_id"=>$dadosJSON["categorias"]["relacao_id"], "status"=>"1"));
                ...
                if ($dadosJSON["categorias"]["relacao_idAtual"]!=$dadosJSON["categorias"]["relacao_id"]) {
                    $dados["resultados"]["ordem"] = proximaOrdem("categorias", array("modulo"=>$moduloVerifica, "relacao_id"=>$dadosJSON["categorias"]["relacao_id"], "status"=>"1"));
                }
    ...
    public function excluirCategoria($dadosJSON)
    {
        postRequest();
        $dadosJSON=readJSON($_POST["ajaxACK"]);
        protectedArea();
