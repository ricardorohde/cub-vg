...
class ACKuploads_Controller
{
    ...
    public function enviar_arquivos($dadosJSON)
    {
        if ($dadosJSON["modulo"] == "categorias") {
            $dadosJSON["modulo"] = "categorias_produtos";
        }
        ...
        if ($dadosJSON['tipo'] == 'imagem') {
            if ($dadosJSON['multi']=="false") {
                ...
                foreach ($dadosJSON["idiomas"] as $idioma => $visivel) {
                    /**
                     * ###################################################################################
                     * removido o título  por pedido do vini                      *###################################################################################
    ...
    public function carregar_arquivos($dadosJSON)
    {
        if ($dadosJSON["modulo"] == "categorias") {
            $dadosJSON["modulo"] = "categorias_produtos";
        }
        ...
        if ($dadosJSON['tipo'] == 'imagem') {
            ...
            if ($imagens) {
                foreach ($imagens as $imagem) {
                    ...
                    $retorno['galeria'][$imagem["id"]]=array('arquivo'=>$caminhoImagem, 'titulo'=>$imagem["titulo_".$idioma[0]["abreviatura"]], 'ordem'=>$imagem["ordem"],  'tamanho'=>$tamanho,"cover"=>$imagem["cover"]);
                }
            } else {
                $retorno['galeria']="";
            }
    ...
    public function excluir_itemUpload($dadosJSON)
    {
        global $endereco_fisico;
        $modelSite=new ACKsite_Model();
        $modulo=$modelSite->idModulo($dadosJSON["modulo"]);
    ...
    public function editar($dadosJSON)
    {
        global $endereco_fisico;
        global $endereco_site;
        //Pega o ID do módulo
    ...
    public function salvar($dadosJSON)
    {
        global $endereco_fisico;
        global $endereco_site;
        //Pega o ID do módulo
        ...
        if ($dadosJSON['tipo'] == 'imagem') {
            ...
            if (count($dadosJSON["idiomas"])>0) {
                ...
                $dadosFoto["resultados"]["cover"]=$dadosJSON['cover'];
                //Faz o update no banco
                dbUpdate("fotos", $dadosFoto);
            }
    ...
    public function cadastro_url($dadosJSON)
    {
        //Pega o ID do módulo
        $modelSite=new ACKsite_Model();
        $modulo=$modelSite->idModulo($dadosJSON["modulo"]);
        ...
        foreach ($dadosJSON["idiomas"] as $idioma => $visivel) {
            ...
            if (!isset($idioma) || $idioma=='undefined') {
                $idioma = System_Language::current();
            }
