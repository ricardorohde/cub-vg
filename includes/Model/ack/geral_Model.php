...
class geral_Model {	
	...
	function listaDestaques() {
		...
		global $db;
		$mysql = $db->prepare('SELECT * FROM destaques WHERE status="1" ORDER BY ordem;');
		$mysql->execute();
	...
	function listaTopicos($apartir,$limite=false,$verificaBotao=true) {
		...
		// Executa o comando no banco de dados
		global $db;
		$mysql = $db->prepare('SELECT * FROM institucional WHERE status<>"9" ORDER BY categoria ASC, ordem ASC '.$apartir.$limite.';');
		$mysql->execute();
		$retorno = $mysql->fetchAll();
		if (count($retorno)>0) {
			return $retorno;
	...
	####################################################################
	function qtdadeItens($tabela,$categoria,$total=false,$retornaValores=false) {
		// Executa o comando no banco de dados
		global $db;
		$mysql = $db->prepare('SELECT * FROM '.$tabela.' WHERE status<>"9";');
		$mysql->execute();
		$resultados = $mysql->fetchAll();	
		if ($retornaValores) {
			return $resultados;
	...
	function listaItens($tabela,$apartir,$limite=false,$verificaBotao=true,$categoria=false, $orderBy=false) {
		...
		// Executa o comando no banco de dados
		global $db;
		$mysql = $db->prepare('SELECT * FROM '.$tabela.' WHERE status<>"9" '.$orderWhere.' '.$apartirWhere.';');
		$mysql->execute();
		$resultados = $mysql->fetchAll();
		if ($categoria) {
			$produtos=array();
	...
	function listaContatos($apartir,$limite=false,$verificaBotao=true,$lidos=false) {
		...
		// Executa o comando no banco de dados
		global $db;
		$mysql = $db->prepare('SELECT * FROM contatos WHERE status<>"9" '.$lidosWhere.' ORDER BY lido ASC, data DESC '.$apartir.$limite.';');
		$mysql->execute();
		$resultados = $mysql->fetchAll();
		if (count($resultados)>0) {
			return $resultados;
